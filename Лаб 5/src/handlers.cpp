...
std::string BookingToJson(const Booking& booking) {
    return std::string("{") +
           ...
           "\"active\":" + (booking.active ? "true" : "false") + "}";
}
...
std::string CreateUserHandler::HandleRequestThrow(
    ...
    try {
        ...
        const auto user = storage.addUser(login, auth::HashPassword(password),
...
std::string CreateBookingHandler::HandleRequestThrow(
    ...
    try {
        ...
        auto& storage = Storage::instance();
        const auto rate_limit = storage.checkBookingRateLimit(user_id);
        request.GetHttpResponse().SetHeader("X-RateLimit-Limit",
                                            std::to_string(rate_limit.limit));
        request.GetHttpResponse().SetHeader("X-RateLimit-Remaining",
                                            std::to_string(rate_limit.remaining));
        request.GetHttpResponse().SetHeader("X-RateLimit-Reset",
                                            std::to_string(rate_limit.reset_unix_seconds));
